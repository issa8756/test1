...
def run_encrypt_tool(action, filename=None):
    ...
    command = ['python', 'encrypt_tool.py', action]  # Updated filename
    if filename:
...
def run_nmap_scanner(ips, scan_type, output=None, speed=None, ports=None):
    ...
    command = ['python', 'nmap_scanner_tool.py', ips]  # Updated filename
...
def run_brute_force_tool(ip, attack_type, passwords_file, username=None, usernames_file=None, threads=False):
    ...
    command = ['python', 'brute_force_tool.py', ip, attack_type, passwords_file]  # Updated filename
    if username:
